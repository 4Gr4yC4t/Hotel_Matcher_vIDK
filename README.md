...
Входной CSV (`POST /match`):

| Колонка | Обязательна | Описание |
|---------|-------------|----------|
| `hotel_id` (или `id`) | да | Уникальный ID отеля |
| `name` | да | Название отеля |
| `lat` | да | Широта |
| `lon` | да | Долгота |

Лишние колонки (например, `address`, `city_id`) игнорируются.

Выходной CSV:

| Колонка | Описание |
|---------|----------|
| `hotel_id` | ID внешнего отеля из запроса |
| `duplicate_hotel_id` | ID совпавшего отеля из базы (или `null`) |
| `score` | Оценка совпадения 0.0–1.0 |
