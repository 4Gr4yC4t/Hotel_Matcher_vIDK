...
1. При запуске загружается эталонная база отелей из `01_hotels_base.csv` (1000 отелей).
2. Имена всех базовых отелей предобрабатываются один раз при старте (транслитерация + lowercase) — это ускоряет каждыйsequent запрос.
3. Пользователь отправляет POST-запрос на `/match` с CSV-файлом внешних отелей (колонки: `name`, `lat`, `lon`, опционально `id`/`external_id`).
4. Для каждого внешнего отеля:
   - Имя предобрабатывается один раз (транслитерация + lowercase).
   ...
   - Сравнение уже подготовленных строк через `SequenceMatcher`.
   - Итоговая оценка: **0.7 × текстовое сходство + 0.3 × географическое сходство**.
5. Невалидные строки (пустые/нечисловые координаты) обрабатываются без падения сервера.
6. Возвращается CSV с колонками `external_id, hotel_id, score`.
...
Каждый ответ сервера содержит заголовок `X-Processing-Time-Ms` — время обработки запроса в миллисекундах. Клиент выводит это значение в консоль при получении ответа.

```bash
curl -v -X POST http://localhost:8000/match \
  -F "file=@external_hotels.csv" \
  -o matches.csv
# В заголовках ответа: < X-Processing-Time-Ms: 1234.56
```
